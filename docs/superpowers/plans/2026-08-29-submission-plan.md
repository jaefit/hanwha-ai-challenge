...
- [x] 백테스트 스크립트화 `src/backtest.py` → `data/derived/backtest.json` (8/30). A in-sample **0.24/0.29**, 등급 적중 88%/94%. **B cross-year(다른 해 E·곡선으로 예측 = 2026 조건) 0.44/0.44**, 등급 76%/88% — 연도 간 규모 이동(여의도(5) 21시 17.8k↔12.5k)이 주 오차. pytest 회귀 가드 추가
      **(9/2 정정)** 이 수치는 `4b10758` 시점 값이다. 이후 `52ff59a`(통제 시간대 도착분을 해제 후 첫 개방 시간대로 이월)로 `backtest.json` 이 재생성되면서 바뀌었다 — **A in-sample 승차오차 0.245/0.214**(2024/2025)·등급적중 82%/100%, **B cross-year 0.420/0.438**·77%/88%. 인용은 `data/derived/backtest.json` 에서 한다
...
### Task 5: 가상 설문 1,000건 + 분석 — **(2026-08-31 드랍)**

> **드랍 사유**: 프로젝트 강점이 "모든 수치 실측 출처"인데 합성 설문은 이와 상충 — 심사자 신뢰 역효과 위험.
> 항목 3(현재 상황·불편)은 실측(여의나루 하차 0·여의도 12~17배·지오비전 '22 연령별·KT 성연령)과 보도 인용으로 대체.
> 대체 작업은 T10 편집기 초안에 흡수: "페인포인트 근거 절 + 실측 차트 3장" (~1시간). 아래 원계획은 기록으로 유지.
...
## 계획 밖 작업 (2026-09-01, 사후 기록)

계획에 없던 하루가 있었다. 태스크로 세우지 않고 진행했으므로 여기 기록만 남긴다 — 커밋 15건(`230bc14`…`f7a3c59`), 테스트 26 → 45건.

| 한 일 | 왜 | 결과 |
|---|---|---|
| **자체 적대적 검증 1회차** | 실전 전에 우리 것을 우리가 먼저 깨본다 | 결함 18건(치명 5·높음 5·중간 7·낮음 1)을 `파일:줄` 근거와 함께. **문서 `redteam-20260901.md` 가 05 의 결함 대장이 됐다** |
| 즉시 5건 + H2·H5 조치 (TDD) | 발행이 조용히 멈추고(C2) 수집기가 죽어도 아무도 모르는(C3) 경로가 9/5 에 그대로 터진다 | `31506fc`. 신규 15건 중 13건 red→green |
| 조치가 만든 실패 모드 2건 재조치 | 자기 수정이 만든 위험은 원래 없던 것이라 남기면 순손실 | `b582279` — rebase 잔류, 워치독 크래시 루프 |
| 팀원 핸드오프 편입 | 보행 경로탐색이 들어왔다 | `e0ccb54`, 3-way 병합 충돌 0. 기준 문서 `routing/README.md`. **이탈 1건(도보 소요 계산)은 미결** |
| **혼잡장 GP 회귀** | CCTV 23대는 점이고 신뢰도가 제각각이라, 믿기 어려운 관측을 버리는 대신 노이즈 σ 로 눌러 넣는 쪽이 맞다 | `8d605a7`~`9d0ab61`(8커밋). `docs/app/field.js` — 순수 함수라 `tests/field_spec.mjs` 가 배포본을 node 로 직접 검증 |
| 경로 찾기 복구 + 회귀 가드 | 위 작업 중 내 정규식 패치가 `RouteHeap`·`routeAstar` 를 삼켰고 문법 검사는 통과해 네 커밋 동안 몰랐다 | `f7a3c59`. 규칙은 `CLAUDE.md`, 사고 기록은 `redteam-20260901.md` "추가 (2026-09-02)" |

**동결 전 결정 2건이 여기서 남았다**: C1 α 밴드 클램프 · 전야제 합격선의 의미. 스펙 §7 참조.

## 백로그 (프리즈 후·제출 후)

- stcis.go.kr 교통카드 빅데이터 — 신길 1호선(코레일) 승차 확보 리드. 접근 신청 절차 확인 필요. 확보 시 exit_shares 입력 데이터만 교체(코드 불변)
- **혼잡장 후속**: 사전 평균을 공간 함수 m₀(x) 로(빈 곳을 모델 예측으로 채움 — 예측 레이어 결합이라 별도 승인 필요) · 한강을 건너 번지지 않게 보행 네트워크 거리 커널 · 길이척도·σ 를 LOO 교차검증으로 교체(9/5 관측 필요)
- ~~**경로탐색 후속**: `DETOUR=1.3` 우회계수를 실측 보정(이벤트광장→여의도역 직선 1,109m vs 실제 1,820m = 1.64배)~~ → **(2026-09-03) 근거 철회.** 재현값은 1,471m·1.31배로 `DETOUR=1.3` 과 거의 같다(M13). 보정할 것이 없다. 표본이 한 구간뿐이라 9/5 이후 여러 구간으로 재확인만 남긴다
- 2회차 적대적 검증: 미감사 `collector_cctv.py`(1순위) · `docs/app/field.js` · `routing/`
