...
**주제: 서울세계불꽃축제(9/5) 관람객 혼잡·귀가 내비 「불꽃배웅」 — 눈부신 밤의 끝, 집으로 가는 길까지.** 9/8(화) 20:00 제출(마감 22:00). 기준 문서는 [topic-fireworks.md](topic-fireworks.md), 실행 계획은 [플랜](docs/superpowers/plans/2026-08-29-submission-plan.md)이다.

행사 사흘 전이다. **예측 모델·수집 파이프라인·공개 대시보드는 돌아가는 상태**이고, 남은 일은 당일 운영(9/4 리허설 · 9/5 실전)과 그 결과 정리다. **코드 동결 2단계**(2026-09-03 변경) — 9/4 17:00 기능 동결 · 9/5 10:00 최종 동결. 이후 `src/`·`docs/index.html` 변경은 hotfix만.
...
**주제 확정 — 서울세계불꽃축제(9/5) 관람객 혼잡·귀가 내비 「불꽃배웅」(이름 2026-09-05 결정).** 마감 9/8(화) 22:00. 기준 문서는 `topic-fireworks.md`.
...
| 9/3(목) | **11:30~13:00 테스트런**(H9·H10) · 중간 정리 5장 `submission/midterm_5pages.md` |
| 9/4(금) | 오전 오프라인 피드백 · **17:00 기능 동결** · 런북 작성 · 저녁 전야제 리허설 수집 |
| 9/5(토) | **10:00 최종 동결** · 12~24시 실전 |
...
기능 동결(9/4 17:00) 전 결정 2건이 남아 있다 — **C1 α 밴드 클램프**(관측 20건에 밴드가 ±5%로 좁아지는데 백테스트 오차는 ±42%)와 **전야제 합격선**(1.9배 미달 시 행동 미정의). 근거는 `redteam-20260901.md` §C1 과 `topic-fireworks.md` §9.
