...
- 표시 억제 규칙: Google은 데이터 부족 시 미표시. 우리는 "수집 전" 상태만 있고 CCTV 야간 저신뢰 억제 규칙 없음.
  **(2026-09-01 정정)** 혼잡장에서는 억제 대신 **가중**으로 처리한다(§4-8) — 저신뢰 관측은 노이즈 σ 를 2.5배로 키워 넣고, 결과의 불확실성 s(x) 가 화면 진하기로 드러난다. 관측이 오래될수록 σ 가 연속적으로 커지므로 "몇 분 지나면 끊는" 임계값이 필요 없다. 다만 **불확실한 곳을 회색으로 덮던 처리를 사용자 요청으로 뺐다** — 색이 진하지 않을 뿐이라 낙관 편향이 남는다(발표 시 불투명도·배지로 설명).
...
8. **(2026-09-01 추가) 점 관측을 면으로 — 가우시안 과정 회귀.** 위 6번이 남긴 문제는 "카메라 5대만 명/m² 로 읽히고 나머지 18대는 등급뿐인데 그 사이 공간은 아무 값도 없다"는 것이었다. 서울 도시데이터는 **핫스팟 1곳당 값 하나**를 주므로 지점 사이가 비어 있다(암스테르담 CMSA 는 밀도·히트맵까지 내지만 방법이 공개돼 있지 않아 대조가 안 된다). 우리는 모든 관측을 명/m² 축으로 환산한 뒤 **Matérn 3/2 커널 가우시안 과정 회귀**로 하나의 면을 만든다(`docs/app/field.js`). 관측 신뢰도는 배제 기준이 아니라 **관측 노이즈 σ**(이분산)로 들어가므로, 배경차분이 실패한 카메라도 버리지 않고 눌러서 반영한다. 사전 평균은 관측 수준에서 경험적 베이즈로 잡는다(고정 사전은 관측을 사전 쪽으로 끌어내려 실제 혼잡을 지웠다).
   **한계**: 길이척도 150m·1200m 와 σ 값(0.10/0.20/0.30)은 **가정이고 출처가 없다** — 9/5 관측이 쌓이면 LOO 교차검증으로 정한다. 거리도 유클리드라 한강 건너로 번진다(보행망 거리 커널이 정답, 제출 후). **같은 방식을 쓰는 운영 시스템은 아직 대조하지 못했다** — 2회차 조사 과제.
...
- CCTV 야간 테스트(2026-08-29 토 20:00~21:00, TOPIS 23대, 60초 주기 1,380건): 스트림 ok 95~100%, 프레임 중앙 밝기 56~98 — 저조도 임계(40) 걸린 카메라 0대(가로등·교량 조명). count 중앙값 1~14명, 플래그 count_vs_occ 4대(63빌딩·국회·노량진수산·마포대교북단 — 차량·구조물 오검출 의심), no frame 산발 17건(재시도로 복구). 축제일 조명(불꽃·인파 밀집)은 미검증 → 임계는 유지, ROI 캘리브레이션(5대) 전까지 모델 입력 제외 유지.
  **(2026-09-02 정정) ROI 5대는 측정 완료**(63빌딩·마포대교남단·마포대교북단·여의공원·원효대교남단, `data/derived/topis_yeouido_cams.json` 의 `roi_m2`) + HD 720p 스트림 9대(`hls_hd`, SD 폴백). 이 5대는 명/m² 로 읽히고 모델·혼잡장에 들어간다. 나머지 18대는 여전히 등급만이고, 혼잡장에서는 σ 를 키워 반영한다(§4-8).
